...
**Capper** is a token-first React design system built on Brad Frost's atomic design principles.
...
| Package | Role |
|---------|------|
| `@capper/tokens` | CSS custom properties: primitives + semantic tokens, light/dark theming |
| `@capper/react` | React atoms: `Box`, `Text`, `Button`, `Input`, `Link`, `Badge`, `Alert`, `Icon`, `InlineCode`, `CodeBlock`, `Stack`, `Inline`, and molecule `Card` |
| `@capper/docs` | Documentation site + Storybook |
...
- Components and docs site styles use **semantic tokens only**. Primitives stay inside `@capper/tokens`.
...
- Generic reusable UI belongs in `@capper/react`. Page-specific docs layout belongs in `@capper/docs`.

### Docs site copy

- **No em dashes** (Unicode U+2014) in user-facing docs copy. Use commas, periods, colons, or hyphens instead.
- Theme switcher: Light / Dark / System. Preference persists in `localStorage` (`capper-theme`).
...
- In dev, `@capper/docs` aliases `@capper/react` to source for fast iteration.
...
| Do | Don't |
|----|--------|
| Use semantic tokens in component styles | Reference primitive tokens outside `@capper/tokens` |
